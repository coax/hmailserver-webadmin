...
if (hmailGetAdminLevel() == 2)
{
	...
	$DomainsCount = $obDomains->Count();

	$dtree .= "d.add(" . $dtitem++ .",0,'" . GetStringForJavaScript("Domains") . " ($DomainsCount)','index.php?page=domains','','','" . "images/world.png','" . "images/world.png');\r\n";
	...
	$RoutesCount = $obRoutes->Count();
	$dtree .= "d.add(" . $settings_smtp_route . ",$settings_smtp_root,'" . GetStringForJavaScript("Routes") . " (" . $RoutesCount . ")','index.php?page=routes','','','" . "images/folder.png','" . "images/folder.png');\r\n";
	...
	$IpRangesCount = $obSecurityRanges->Count();
	...
	$dtree .= "d.add(" . $settings_ipranges_root . ",$advanced_root,'" . GetStringForJavaScript("IP Ranges") . " ($IpRangesCount)','index.php?page=securityranges','','','" . "images/folder.png','" . "images/folder.png');\r\n";

	for ($j = 0; $j < $IpRangesCount; $j++)
	{
		$obSecurityRange = $obSecurityRanges->Item($j);

		$rangename = HMEscape($obSecurityRange->Name);
		$rangename = PreprocessOutput($rangename);
	...
	$IncomingRelaysCount = $obIncomingRelays->Count();

	$settings_incoming_relays_root = $dtitem++;
	$dtree .= "d.add(" . $settings_incoming_relays_root . ",$advanced_root,'" . GetStringForJavaScript("Incoming relays") . " ($IncomingRelaysCount)','index.php?page=incomingrelays','','','" . "images/folder.png','" . "images/folder.png');\r\n";

	for ($j = 0; $j < $IncomingRelaysCount; $j++)
	{
		$obIncomingRelay = $obIncomingRelays->Item($j);
		$relayname = HMEscape($obIncomingRelay->Name);
...
function GetStringForDomain($obDomain, $parentid)
{
	...
	echo '            <li><a href="index.php?page=domain&action=edit&domainid=' . $obDomain->ID . '" title="' . $domainname . '">' . $domainname . '</a></li>' . PHP_EOL;
